...
public class GetFeature {
	...
	public FeatureCollectionResponse run(GetFeatureRequest request) throws WFSException {
		...
		// stored queries, preprocess compile any stored queries into actual
		// query objects
		processStoredQueries(request);
		queries = request.getQueries();

		if (request.isQueryTypeNamesUnset()) {
			// do a check for FeatureId filters in the queries and update the
			// type names for the
			// queries accordingly
			for (Query q : queries) {
		...
		if (totalOffset == -1 && request.getVersion().startsWith("2") && wfs.isCiteCompliant()) {
			// Strict compliance with the WFS 2.0 spec requires startindex to
			// default to zero.
			// This is not enforced because startindex triggers sorting and
			// reduces performance.
			// The CITE tests for WFS 2.0 do not yet exist; the CITE compliance
			// setting is taken
		...
		// optimization: WFS 1.0 does not require count unless we have multiple
		// query elements
		...
		try {
			for (int i = 0; (i < queries.size()) && (count < maxFeatures); i++) {
				...
				try {
					...
					if (!query.getAliases().isEmpty()) {
						if (query.getAliases().size() != query.getTypeNames().size()) {
							throw new WFSException(request,
									String.format("Query specifies %d type names and %d " + "aliases, must be equal",
					...
					// parse the requested property names and distribute among
					// requested types
					...
					//
					// Validation of filters on non-simple feature types is not
					// yet supported.
					// FIXME: Support validation of filters on non-simple
					// feature types:
					// need to consider xpath properties and how to configure
					...
					if (filter != null) {
						if (meta.getFeatureType() instanceof SimpleFeatureType) {
							if (metas.size() > 1) {
								// sanitize aliases, they must not conflict with
								// feature type names
								...
								// the join extracting visitor cannot handle
								// negated filters,
								// the simplifier handles most common case
								// removing the negation,
								// e.g., not(a < 10) -> a >= 10
								filter = SimplifyingFilterVisitor.simplify(filter);

								// join, need to separate the joining filter
								// from other filters
								...
								// validate the filter for each join, as well as
								// the join filter
								for (int j = 1; j < metas.size(); j++) {
									Join join = joins.get(j - 1);
									if (!isValidJoinFilter(join.getJoinFilter())) {
										throw new WFSException(request,
												"Unable to perform join with specified join filter: " + filter);
									}
						...
						if (!propertyNames.isEmpty()) {
							...
							for (Iterator iter = propertyNames.iterator(); iter.hasNext();) {
								...
								if (propName.evaluate(meta.getFeatureType()) == null) {
									String mesg = "Requested property: " + propName + " is " + "not available " + "for "
							...
							// if we need to force feature bounds computation,
							// we have to load
							// all of the geometries, but we'll have to remove
							// them in the
							...
							// we must also include any properties that are
							// mandatory ( even if not requested ),
							// ie. those with minOccurs > 0
							// only do this for simple features, complex
							// mandatory features are handled by app-schema
							if (meta.getFeatureType() instanceof SimpleFeatureType) {
							...
							// for complex features, mandatory properties need
							// to be handled by datastore.
						}
					...
					FeatureSource<? extends FeatureType, ? extends Feature> source = primaryMeta.getFeatureSource(null,
							hints);
					...
					org.geotools.data.Query gtQuery = toDataQuery(query, filter, offset, queryMaxFeatures, source,
							request, allPropNames.get(0), viewParam, joins, primaryTypeName, primaryAlias);

					// System.out.println("Query is " + query + "\n To gt2: " +
					// gtQuery);

					FeatureCollection<? extends FeatureType, ? extends Feature> features = getFeatures(request, source,
							gtQuery);

					// For complex features, we need the targetCrs and version
					...
					// for each nested feature to have different crs hence we
					// need to reproject on each
					// feature accordingly.
					if (!(meta.getFeatureType() instanceof SimpleFeatureType)) {
					...
					if (!calculateSize) {
						// if offset was specified and we have more queries left
						// in this request then we
...
//						System.out.println("this comment need to be changes back after the testing. $#$#DF$GF654yH^5");
					...
					// if offset is present we need to check the size of this
					// returned feature collection
					// and adjust the offset for the next feature collection
					// accordingly
					if (offset > 0) {
						if (size > 0) {
							...
							// no features might have been because of the offset
							// that was specified, check
							// the size of the same query but with no offset
							org.geotools.data.Query q2 = toDataQuery(query, filter, 0, queryMaxFeatures, source,
									request, allPropNames.get(0), viewParam, joins, primaryTypeName, primaryAlias);

							// int size2 = getFeatures(request, source,
							// q2).size();
					...
					// collect queries required to return
					// numberMatched/totalSize
					// check maxFeatures and offset, if they are unset we can
					// use the size we
					// calculated above
					isNumberMatchedSkipped = meta.getSkipNumberMatched() && !request.isResultTypeHits();
					if (!isNumberMatchedSkipped) {
						if (calculateSize && (queryMaxFeatures == Integer.MAX_VALUE || size < queryMaxFeatures)
								&& offset <= 0) {
							totalCountExecutors.add(new CountExecutor(size));
						} else {
							org.geotools.data.Query qTotal = toDataQuery(query, filter, 0, Integer.MAX_VALUE, source,
									request, allPropNames.get(0), viewParam, joins, primaryTypeName, primaryAlias);
							totalCountExecutors.add(new CountExecutor(source, qTotal));
						}
					}

					// we may need to shave off geometries we did load only to
					// make bounds
					// computation happy
					// TODO: support non-SimpleFeature geometry shaving
					List<PropertyName> metaPropNames = propNames.get(0);
					if (features.getSchema() instanceof SimpleFeatureType && metaPropNames != null
						...
						SimpleFeatureType targetType = DataUtilities
								.createSubType((SimpleFeatureType) features.getSchema(), residualNames);
						features = new FeatureBoundsFeatureCollection((SimpleFeatureCollection) features, targetType);
					}

					// JD: TODO reoptimize
					// if ( i == request.getQuery().size() - 1 ) {
					// //DJB: dont calculate feature count if you dont have to.
					// The MaxFeatureReader will take care of the last iteration
					// maxFeatures -= features.getCount();
					// }

					// GR: I don't know if the featuresults should be added here
					// for later
					// encoding if it was a lock request. may be after ensuring
					// the lock
					// succeed?
					results.add(features);
				} catch (WFSException e) {
					// intercept and set locator to query handle if one was set,
					// or if it simply set
			...
			// total count represents the total count of the features matched
			// for this query in cases
			// where the client has limited the result set size, so we compute
			// it lazily
			if (isNumberMatchedSkipped) {
				totalCount = BigInteger.valueOf(-1);
			} else if (count < maxFeatures && calculateSize) {
				// optimization: if count < max features then total count ==
				// count
				totalCount = BigInteger.valueOf(count);
			} else {
				// ok, in this case we're forced to run the queries to discover
				// the actual total count
				// We do so lazily, not all output formats need it, leveraging
				// the fact that BigInteger
				...
				enhancer.setCallback(new LazyLoader() {
					...
					public Object loadObject() throws Exception {
						...
						for (CountExecutor q : totalCountExecutors) {
							...
							// if the count is unknown for one, we don't know
							// the total, period
							if (result == -1) {
								totalCount = -1;
								break;
	 ...
	 */
	protected FeatureCollectionResponse buildResults(GetFeatureRequest request, int offset, int maxFeatures, int count,
			BigInteger total, List results, String lockId) {
		...
		if (offset > 0 || count < Integer.MAX_VALUE) {
			...
			// get the Request thread local since we need to know about the
			// request, whether it is
			...
			// TODO: try to encode the request as best we can in a GET request,
			// only issue should
			// be the filter and encoding it property... especially for joins
			// that might be
			// tricky, and it also may cause the request to be too large for a
			// get request
			...
			if (offset > 0) {
				...
				// previous offset calculated as the current offset -
				// maxFeatures, or 0 if this is a
				// negative value
	...
	void encodeQueryAsKvp(Query q, StringBuilder typeNames, StringBuilder propertyName, StringBuilder aliases,
			StringBuilder filter, boolean useDelim) {
		...
		// filter
		if (filter != null) {
			// TODO: check the length of the encoded filter and ensure it does
			// not put us over the
	 ...
	 * @throws IOException
	 */
	protected FeatureCollection<? extends FeatureType, ? extends Feature> getFeatures(Object request,
			FeatureSource<? extends FeatureType, ? extends Feature> source, org.geotools.data.Query gtQuery)
			throws IOException {
	 ...
	 *
	 * <p>
	 * if maxFeatures is a not positive value Query.DEFAULT_MAX will be used.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * @param maxFeatures
	 *            number of features, or 0 for Query.DEFAULT_MAX
	 *
	...
	public org.geotools.data.Query toDataQuery(Query query, Filter filter, int offset, int maxFeatures,
			...
			List<PropertyName> props, Map<String, String> viewParams, List<Join> joins, QName primaryTypeName,
			String primaryAlias) throws WFSException {
		...
		// make sure every bbox and geometry that does not have an attached crs
		// will use
		...
		org.geotools.data.Query dataQuery = new org.geotools.data.Query(typeName.getLocalPart(), transformedFilter,
		...
		if (traverseXlinkDepth != null) {
			// TODO: make this an integer in the model, and have hte
			// NumericKvpParser
			// handle '*' as max value
		...
		// tell the datastore to use a lite coordinate sequence factory, if
		// possible
		...
		if (request.getSimplifyMethod().equalsIgnoreCase(GetFeatureTypeImplExt.SIMPLIFYMETHOD_DP)) {
			...
			System.out.println(
					"Here in org.geoserver.wfs.GetFeature.toDataQuery we wrap the distance torlance parameter into query hints");
			System.out.println(String.format("simplify method: %s; distance tolerance: %f", request.getSimplifyMethod(),
					distanceTolerance));
		}
	...
	boolean isValidJoinFilter(Filter filter) {
		PostPreProcessFilterSplittingVisitor visitor = new PostPreProcessFilterSplittingVisitor(joinFilterCapabilities,
				null, null);
	...
	List<List<String>> parsePropertyNames(Query query, List<FeatureTypeInfo> featureTypes) {
		...
		// go through all property names and distribute based on prefix
		// accordingly
				...
				fvisitor = new AbstractFilterVisitor() {
					public Object visit(BBOX filter, Object data) {
						if (filter.getSRS() != null && !fquery.getSrsName().toString().equals(filter.getSRS())) {
							...
							// back project bounding box into geographic
							// coordinates
							CoordinateReferenceSystem geo = DefaultGeographicCRS.WGS84;

							GeneralEnvelope e = new GeneralEnvelope(new double[] { filter.getMinX(), filter.getMinY() },
							...
							if (e.getMinimum(0) < valid.getWestBoundLongitude()
								...
								throw new WFSException(request, "bounding box out of valid range of crs",
										"InvalidParameterValue");
							}
						}
		...
		if (path.contains("/")) {
			...
			if (path.contains(":")) {
				...
				return filterFactory
						.property(new NameImpl(namespaceContext.getURI(path.substring(0, i)), path.substring(i + 1)));
			} else {
